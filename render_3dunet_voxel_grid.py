...
from thre3d_atom.thre3d_reprs.voxels_3dunet import create_3dunet_voxel_grid_from_saved_info_dict
...
@click.option("-r", "--trained_rf_path", type=click.Path(file_okay=True, dir_okay=False),
              required=True, help="path to the trained (reconstructed) model")
...
def main(**kwargs) -> None:
    ...
    unet_based_model_path = Path(config.model_path)
    pretrained_rf_path = Path(config.trained_rf_path)
    output_path = Path(config.output_path)

    # create the output path if it doesn't exist
    output_path.mkdir(exist_ok=True, parents=True)

    # load pretrained ReLU fields
    pretrained_vol_mod, _ = create_volumetric_model_from_saved_model(
        model_path=pretrained_rf_path,
        thre3d_repr_creator=create_voxel_grid_from_saved_info_dict,
        device=device,
    )

    # load volumetric_model from the model_path
    unet_vol_mod, extra_info = create_volumetric_model_from_saved_model(
        model_path=unet_based_model_path,
        thre3d_repr_creator=create_3dunet_voxel_grid_from_saved_info_dict,
        device=device,
    )

    # set up unet based model
    unet_vol_mod.thre3d_repr.set_input_grid(pretrained_vol_mod.thre3d_repr._densities,
                                           pretrained_vol_mod.thre3d_repr._features)
    _, _  = unet_vol_mod.thre3d_repr.get_densities_and_features()
